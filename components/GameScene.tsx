...
const AnimatedPlayer: React.FC<{ player: Player; localPlayer: Player | undefined; visionRadius: number }> = ({ player, localPlayer, visionRadius }) => {
  ...
  // Use a ref to track the smoothed position to avoid jitter
  const smoothedPos = useRef(new THREE.Vector3(player.pos.x, player.isAlive ? 0 : 0.8, player.pos.y));

  useFrame((state, delta) => {
    if (!groupRef.current) return;
    
    // Smoothly interpolate position
    const targetY = player.isAlive ? 0 : 0.8;
    const targetPos = new THREE.Vector3(player.pos.x, targetY, player.pos.y);
    
    // If it's the local player, follow more strictly but still smoothly
    // If it's a remote player, interpolate to hide network jitter
    const lerpFactor = isLocal ? 0.3 : 0.15;
    smoothedPos.current.lerp(targetPos, lerpFactor);
...
const SceneContent: React.FC<GameSceneProps> = ({ players, localPlayerId, tasks, deadBodies, onEmergencyPress, mapData, visionRadius = 15, activeSabotage }) => {
  ...
  // Set background and fog immediately
  useEffect(() => {
    if (!scene) return;
  ...
  useFrame((state, delta) => {
    if (localPlayer) {
      ...
      const lookTarget = new THREE.Vector3(localPlayer.pos.x, 0, localPlayer.pos.y);
      
      // Use frame-rate independent lerp for smoother camera
      const lerpFactor = 1 - Math.pow(0.01, delta);
      smoothedCameraTarget.current.lerp(targetPos, lerpFactor);
      smoothedLookAt.current.lerp(lookTarget, lerpFactor);
      
      camera.position.copy(smoothedCameraTarget.current);
  ...
  return (
    <>
      ...
      {mapData.rooms.map((room) => (
        ...
        <Text key={room.name} position={[room.pos.x, 0.05, room.pos.y]} rotation={[-Math.PI / 2, 0, 0]} fontSize={2.5} color="#0f172a" fillOpacity={0.15} anchorX="center" anchorY="middle">{room.name.toUpperCase()}</Text>
      ))}
...
const GameScene: React.FC<GameSceneProps> = (props) => {
  return (
    <div className="w-full h-full bg-[#94a3b8]">
      <Canvas 
        shadows 
        camera={{ fov: 45, position: [0, 25, 25], near: 0.1, far: 2000 }} 
